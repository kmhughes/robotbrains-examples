...
public class NettyStaticContentHandler implements NettyHttpGetRequestHandler,
  ...
  public void handleWebRequest(ChannelHandlerContext ctx, HttpRequest request,
    ...
    if (cookiesToAdd != null) {
      ...
      addHeader(response, HttpHeaders.Names.SET_COOKIE,
          ServerCookieEncoder.STRICT.encode(Collections2.transform(cookiesToAdd,
              new Function<HttpCookie, Cookie>() {
                @Override
    ...
    if (HttpMethod.GET == request.getMethod()) {
      if (ch.pipeline().get(SslHandler.class) != null) {
        ...
        writeFuture.addListener(new ChannelProgressiveFutureListener() {
          ...
          @Override
          public void
              operationProgressed(ChannelProgressiveFuture future, long progress, long total)
                  throws Exception {
            // Do nothng
          }
        });
