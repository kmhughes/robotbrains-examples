  ...
  private void handleHttpRequest(ChannelHandlerContext ctx, HttpRequest req) {
    try {
      ...
      if (authProvider != null) {
        ...
        if ((response == null) || !response.authSuccessful()) {
          if ((response == null) || response.redirectUrl() != null) {
            ...
            webServer.getLog()
                .warn("Auth requested and no redirect available for %s", req.getUri());
      ...
      if (handleWebGetRequest(ctx, req, response)) {
        ...
        String message =
            String
                .format("HTTP [%d] %s --> (No handlers for request)", status.code(), req.getUri());
  ...
  private boolean handleWebPostRequest(ChannelHandlerContext context, HttpRequest request,
    ...
    try {
      NettyHttpFileUpload fileUpload =
          new NettyHttpFileUpload(request, new HttpPostMultipartRequestDecoder(HTTP_DATA_FACTORY,
              request), postRequestHandler, this, cookies);
